...
class CylinderWindow(BaseWindow):
    ...
    def initUI(self) -> None:
        image_label = QLabel()
        image_label.setPixmap(QPixmap("images/cylinder_equation.jpg"))
        ...
        home_page_button = QPushButton("Anasayfa")
    ...
    def calculate(self) -> None:
        ...
        if r and h and pi:
            self.output_line_edits["floor_area"].setText(str(2 * pi * r ** 2))
            self.output_line_edits["lateral_area"].setText(str(2 * pi * r * h))
            self.output_line_edits["total_surface_area"].setText(
                str(2 * pi * r * (r + h)))
            self.output_line_edits["cross_sectional_area"].setText(
                str(pi * r ** 2))
            self.output_line_edits["volume"].setText(str(pi * r ** 2 * h))
        else:
